...
############### description
# If W = 4*V^2 with V ~ Kolmo then W ~ LogisKolmo i.e. a logistic Kolmogorov distribution.
# Indeed if W ~ LogisKolmo and X|W ~ N(0,W)  then X ~ Logis(0,1). 
#
# This function generates random numbers from logistic Kolmogorov given a single value of
# logistic distribution, i.e. W|X is the posterior logistic Kolmogorov.
# 
# The exact simulation is performed through an acceptance-rejection algorithm. The
# auxiliary, or proposal, density is an inverse gamma distribution. The rate parameter of 
# the inverse gamma is set to 0.5 * pi^2, the shape parameter is set to  
# 0.5 * ( 1 + (pi^2 + x^2) / E(W|X) ). In this way the target and the proposal have the
# same mean.
#
# The probability of acceptance i.e. 1/M = iM is
#
#  iM = pre_iM / ( dt(x*sqrt(shape/rate), df = 2*shape)*sqrt(shape/rate) ) * dlogis(x).
#
...
#                   firstMaj = exp( 
#                                  log(8.857) + lgamma(shape) - shape*log(rate) +
#                                  shape*log(vEqual)
#                                 )
#                   secondMaj = exp(
#                                   lgamma(shape) - shape*log(rate) +
#                                   (1+shape)*log(vStar) + rate/vStar - 0.5*vStar
#                                  )
#                   pre_iM = 1 / max( firstMaj, secondMaj )
#
# Notice that the value of vEqual must not be changed, otherwise the values of vStar,
...
rpostlogiskolmo = function(n, x){
  ...
  if(n<lenX0){
    ...
    if(nMODlenX0>0){
      ...
      if( is.null(dim(x)) ){
        # fill missing simulations with NA
        res = matrix(c(res, rep(NA, lenX0-nMODlenX0)), nrow = lenX0, ncol = nINTlenX0+1)
        # report a warning
        warning(paste("length of x is not a multiple of the number of simulations, ",
        ...
        res = array(c(res, rep(NA, lenX0-nMODlenX0)), dim = c(dim(x),nINTlenX0+1))
        # report a warning
        warning(paste("length of x is not a multiple of the number of simulations, ",
                      "there are some NA in the results array", sep = ""))
        # check if res is a 2D array
